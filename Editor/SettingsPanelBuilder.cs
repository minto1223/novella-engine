...
public class SettingsPanelBuilder
{
    ...
    public static void Build()
    {
        ...
        // --- SettingsUIController の参照を配線 ---
        var novellaManager = GameObject.Find("NovellaManager");
        var settingsUI = novellaManager != null ? novellaManager.GetComponent<SettingsUIController>() : null;
        if (settingsUI != null)
        {
            var so = new SerializedObject(settingsUI);
            so.FindProperty("_panel").objectReferenceValue            = panelTr.gameObject;
