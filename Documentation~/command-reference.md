...
シナリオJSON/CSVの `commands` 配列に記述するコマンド一覧です。全41種。
...
### テキスト・表示
- [say](#say--セリフ表示) / [show_title](#show_title--章タイトル表示)
...
| 書式 | 意味 | 例 |
|---|---|---|
| `flag_name` | フラグが truthy（`"true"` / `"1"` / `"yes"`） | `"met_alice"` |
| `!flag_name` | フラグが falsy（未設定 or 上記以外） | `"!met_alice"` |
| `A == B` | 一致（両辺が数値なら数値、それ以外は文字列として比較） | `"route == good"` |
| `A != B` | 不一致 | `"route != bad"` |
| `A >= B` / `A <= B` / `A > B` / `A < B` | 数値比較 | `"affection >= 10"` |
| `A AND B` | 両方が真 | `"affection >= 10 AND friendly"` |
| `A OR B` | どちらかが真 | `"has_key OR has_lockpick"` |
| `( ... )` | 優先順位の指定 | `"(rich OR noble) AND !wanted"` |
| `A + B` / `-` / `*` / `/` / `%` | 整数の四則演算・剰余 | `"hp - damage > 0"` |
| `"文字列"` | 文字列リテラル | `"route == \"good end\""` |

### 評価のルール

- 優先順位は高い順に `!`・単項マイナス → `* / %` → `+ -` → `< <= > >=` → `== !=` → `AND` → `OR`
- `AND` / `OR` / `NOT` は大文字で書きます。`&&` / `||` / `!` も同義です
- 比較・計算の**両辺**にフラグを書けます（`"hp > mp"`、`"gold >= price * 2"`）
- クォートしない語は「同名のフラグがあればその値、なければその語自身」として扱われます。`"route == alice"` は文字列 `alice` との比較になりますが、`alice` というフラグを定義している場合はその値との比較になります。**文字列として比較したいときはクォート**してください
- 計算は整数のみです（小数は切り捨て）。0で割ったときは左辺の値がそのまま使われ、警告ログが出ます
- 解釈できない式は警告ログを出したうえで、従来の単純な評価方式で再解釈されます

`Novella > Validate Scripts` を実行すると、条件式・計算式が構文として解釈できるかを事前にチェックできます。
...
画面全体にパーティクルエフェクトを再生します。**非ブロッキング**（即座に次のコマンドへ進み、セリフと並行して動作）。

| パラメータ | 必須 | デフォルト | 説明 |
|---|---|---|---|
| `value` | - | `sakura` | プリセット名、または `Resources/Particles/` 内のPrefab名 |
| `duration` | - | 0 | 持続時間（秒）。**0で無限**（`stop_particle` で止めるまで継続） |

同名のエフェクトが既に再生中の場合は、いったん破棄してから再生し直します。
`Resources/Particles/{value}` にPrefabがあればそちらが優先され、無ければ下記プリセットが動的生成されます。

**プリセット一覧**

| 名前 | 内容 |
|---|---|
| `sakura` | 桜吹雪。左右に揺れながら回転して落下 |
| `snow` | 雪。sakuraより小粒・低速で降る |
| `rain` | 雨。縦に引き伸ばした粒が高速落下 |
| `firefly` | 蛍。ふわふわ漂いながら明滅する |
| `dust` | 舞う塵。ゆっくり上方向に漂う |

プリセット名にも `Resources/Particles/` のPrefabにも一致しない場合は、警告を出して何も再生しません。

```json
{ "type": "play_particle", "value": "sakura" }
{ "type": "play_particle", "value": "rain", "duration": 10.0 }
```
...
再生中のパーティクルエフェクトを停止します。

| パラメータ | 必須 | デフォルト | 説明 |
|---|---|---|---|
| `value` | - | - | 停止するエフェクト名。**省略すると全エフェクトを停止** |
| `duration` | - | 1.0 | フェードアウト秒数（新規発生を止めてから消えるまでの猶予） |

```json
{ "type": "stop_particle", "value": "sakura" }
{ "type": "stop_particle" }
```
...
スクリプトの再生を終了し、**タイトル画面に戻ります**（`SceneTransitionManager` があればフェードを挟みます）。シーン回想にも自動記録されます。

```json
{ "type": "end" }
```

`label` を指定すると、その名前でエンディングを記録します（エンディングリスト画面に反映されます）。
...
| フィールド | 説明 |
|---|---|
| `label` | エンディング名（任意）。指定するとエンディングリストに到達済みとして記録される |

戻り先のシーン名は `NovellaEngine` の `Scene Flow > Title Scene Name`（既定 `TitleScene`）で変更できます。
...
|---|---|---|---|
| `target` | **必須** | - | 対象フラグ名 |
| `value` | **必須** | - | 演算式（例: `"+5"`, `"-3"`, `"*2"`, `"/2"`, `"=atk * 2 - def"`） |

`value` の先頭が `+` `-` `*` `/` `%` `=` なら複合代入（`"+5"` → `target = target + 5`）、
それ以外は式を評価して代入します。演算子の右側には[条件式と同じ書式](#条件式の書式)の計算式を書けます。
...
{ "type": "calc", "target": "gold", "value": "+reward * bonus" }
```

数式として評価できない値は文字列としてそのまま代入されます（`{ "value": "good_end" }` → `"good_end"`）。
0で割ろうとした場合は対象フラグを変更せず、警告ログを出します。
