...
`TitleCanvas` に設定パネル（`SettingsPanel > SettingsCard`）をゼロから構築します（SampleScene版 Rebuild Settings Panel のTitle版）。

- 内容はSampleScene版と同一（テキスト/サウンド/表示の各セクション、タブ切り替え、初期化・閉じるボタン）
- `TitleManager` に `SettingsUIController` をアタッチし、`_settingsUI` に自動配線
- タイトル画面の「Settings」ボタン（Button Builderで追加）と組み合わせて使用
- `ConfirmDialog` はTitleSceneに未構築のため配線されない（「初期化」ボタンは確認なしでリセットされる）
...
- **機能選択:** ドロップダウンで追加するボタンの機能を選択
  - **Title タブ:** New Game / Continue / Quit / Reset / CG Gallery / BGM Gallery / Scene Recollection / Chapter Select / Ending List / Flowchart / Settings
...
- **ボタン色:** カラーピッカーで背景色を指定
- **スタイル（4状態演出）:** 通常/ホバー/押下/無効の見た目を切り替える `NovellaButton` の適用方法を選択
  - **なし（従来のフラット色）:** 従来どおりの単色ボタン
  - **テーマにおまかせ（推奨・デフォルト）:** `NovellaButton` を装着し、テーマ（`NovellaUITheme`）の `PrimaryButtonStyle`（Title）/ `IconButtonStyle`（Game）が実行時に自動割り当てされる
  - **カスタムスタイル指定:** `Create > Novella > Button Style` で作成したスタイルアセットを個別に指定
...
#### Novella > Diced Character Builder

表情差分の立ち絵PNG群を、重複部分を除いた1枚のアトラスに統合してメモリ使用量を削減します（Dicing方式）。

- **対象**: `Assets/Novella/Resources/Characters/` の `{ID}.png`（基本表情）と `{ID}_*.png`（表情差分）。全画像が同一サイズであること
- **出力**: `Assets/Novella/Resources/Characters/Diced/{ID}.asset`（表情マップ）と `{ID}_atlas.png`（アトラス）
- **使い方**: Character IDを入力してBuild。「検出されたIDを一覧」で候補IDを確認可能
- Cell Size（既定64px）は分割セルの1辺。小さいほど重複排除が効くがセル数が増える
- `_blink` / `_talk` 付きファイルも独立した表情として取り込まれ、まばたき・口パクにそのまま使われる
- 生成後は `show_char` が**自動的に**アトラス版（`DicedImage`）で描画される。シナリオ側の変更は不要
- 表情差分を追加・変更したら再実行すること（既存アセットはGUIDを保ったまま更新される）
...
#### Tools > Novella > Apply Sample Theme Settings Skin

設定パネル（タブ・スライダー・トグル・フッターボタン）に仮テーマ（サンプルの水色テーマ）のスキンを一括適用します。
